...
| Layer | Status | Notes |
|-------|--------|-------|
| Flutter UI (Dart) | ✅ Complete | Screens, navigation, toggle logic |
| Android Gradle scaffold | ✅ Complete | `settings.gradle`, `build.gradle`, `AndroidManifest.xml`, `MainActivity.kt` |
| Kotlin AccessibilityService stub | ✅ Complete | Migrated to `com.reclaim.app` package |
| CI build | ❌ Failing | Plugin SDK version mismatches (see below) |
| APK installable & launchable | ❌ Not yet | Depends on CI fix |
...
but the following plugin(s) require a higher Android SDK version:
  - shared_preferences_android compiles against Android SDK 36  ← OK
  - jni compiles against Android SDK 35                         ← OK
  - jni_flutter compiles against Android SDK 35                 ← OK
```
...
```
✅ **Fixed** — both targets now set to Java 17 in `app/build.gradle`.
...
- [ ] **Platform channel bridge** — create `lib/services/accessibility_bridge.dart`
  ```dart
  // MethodChannel to check if AccessibilityService is enabled
  ...
  ```
- [ ] **MainActivity.kt** — implement the method channel handler:
  ```kotlin
  ...
  ```
- [ ] **Onboarding screen** — prompt user to enable accessibility service (deep link to Settings)
- [ ] **`FocusLockAccessibilityService.kt`** — implement `onAccessibilityEvent` to detect and hide Shorts/Reels UI elements
...
- [ ] **TikTok** — straightforward: full-screen video in `com.zhiliaoapp.musically`
- [ ] **Block action** — use `performGlobalAction(GLOBAL_ACTION_BACK)` or overlay a blocking screen

### Phase 4 — Polish & Release (Est: 1 hr)
...
# 4. Check accessibility service is visible in Settings
